...
const SettingTable = () => {
  ...
  const fetchSettings = async (page: number) => {
    try {
      ...
      const res = await fetch(`${ process.env.NEXT_PUBLIC_ENDPOINT || 'https://192.168.2.5:5050'}/api/global-settings?page=${page}`, {
        headers: {
  ...
  const handleBlur = async (key: string) => {
    ...
    try {
      const token = await session();
      await fetch(`${ process.env.NEXT_PUBLIC_ENDPOINT || 'https://192.168.2.5:5050'}/api/global-setting/`, {
        method: "PUT",
        headers: {
